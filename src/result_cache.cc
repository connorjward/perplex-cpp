...
namespace perplexcpp
{
  ResultCache::ResultCache(const size_t capacity, const double rtol)
    : rtol(rtol)
  {
    this->items = std::list<CacheItem>(capacity);
  }
		   ...
		   const std::vector<double>& composition)
  {
    ...
    {
      // This is short-circuit evaluated so the faster pressure and temperature 
      // evaluations are done first.
  ...
  void
  ResultCache::put(const double pressure, 
		   const double temperature, 
		   const std::vector<double>& composition,
		   const MinimizeResult& result)
  {
    // Add the new item to the front of the list and remove the last item.
    const CacheItem item { pressure, temperature, composition, result };
